...
### Added
- `.gitignore` excluding local runtime state (logs, sessions, todos, cache, telemetry, plans, plugins, etc.), plugin-managed skills (gsap, hyperframes, hyperframes-cli, impeccable, website-to-hyperframes, n8n suite, remotion-best-practices, supabase-postgres-best-practices, vercel-react-best-practices, web-design-guidelines, ui-test, find-skills), MCP auth cache, caveman marker, local settings (`.claude/settings.local.json`), channels, and chrome native host script
